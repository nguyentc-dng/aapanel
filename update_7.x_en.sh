...
if [ $? -ne 0 ];then
    echo "Use backup node to get the latest version number"
    version=$(curl -Ss --connect-timeout 20 -m 60 https://sg1-www.aapanel.com/api/panel/getLatestOfficialVersion 2>/dev/null)
fi

check_version_num=$( echo "$version"|grep -Eo '^[0-9]+' )
if [ "$check_version_num" = '' ] || [ "$version" = '' ];then
...
Set_Centos8_Repo(){
    ...
    if [  "${ALIYUN_CHECK}" ] && [ "${is64bit}" == "64" ] && [ ! -f "/etc/yum.repos.d/Centos-vault-8.5.2111.repo" ];then
        ...
        Wget_File "/etc/yum.repos.d/Centos-vault-8.5.2111.repo" "https://mirrors.aliyun.com/repo/Centos-vault-8.5.2111.repo"
        Wget_File "/etc/yum.repos.d/epel-archive-8.repo" "https://mirrors.aliyun.com/repo/epel-archive-8.repo"
...
Install_Openssl111(){
    ...
    if [[ $os_type = "el" ]] && [[ $os_version == "7" ]] && [[ $openssl_make != "yes" ]]; then
        Wget_File "openssl111.tar.gz" "${download_Url}/install/src/openssl111${CPU_arch}.tar.gz" "-t 2 -T 20"
        ...
        Wget_File "openssl-${opensslVersion}.tar.gz" "${download_Url}/src/openssl-${opensslVersion}.tar.gz" "-t 2 -T 20"
...
# Need to use Werkzeug 2.2.3
    ...
    change_pip_package_list=$( $mypip list | grep -E "Werkzeug|lxml|pycountry|jsonpath" )
...
Install_Python_Lib() {
    ...
    if [ "${os_version}" != "" ]; then
        ...
        Wget_File "$pyenv_file" "$download_Url/install/pyenv/3.12/pyenv-${os_type}${os_version}-x${is64bit}${python_version}.tar.gz" "-t 2 -T 20"
        if [ "$?" != "0" ];then
            Wget_File "$pyenv_file" "$download_Url/install/pyenv/3.12/pyenv-${os_type}${os_version}-x${is64bit}${python_version}.tar.gz" "-t 2 -T 20"
        fi
    ...
    Wget_File "$python_src" "$download_Url/src/Python-${py_version}.tar.xz" "-t 2 -T 20"
...
jsonpath=$($mypip list|grep jsonpath)
if [ "$jsonpath" = "" ];then
	$mypip install --no-cache-dir jsonpath
fi
...
if [ -f "/www/server/panel/--no-check-certificate" ];then
    rm -f "/www/server/panel/--no-check-certificate"
fi
